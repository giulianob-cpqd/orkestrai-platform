...
import { ArrowLeft, Pencil, Users, User, GitBranch, Database, Building2, Server, Cloud, Monitor, Code2 } from "lucide-react";
...
export const Route = createFileRoute("/agents/$id/")({
  ...
  head: ({ loaderData }) => ({
    meta: [
      { title: `${loaderData?.flow.name ?? "Agent"} · OrkestrAI` },
...
function AgentDetail() {
  const { flow: initialFlow } = Route.useLoaderData() as { flow: AgentFlow };
  ...
  useEffect(() => {
    const handleFocus = () => {
      const updatedFlow = getAgentFlow(initialFlow.id);
      if (updatedFlow) {
        setFlow(updatedFlow);
      }
    };
  ...
  const { fanIn, fanOut, rags } = useMemo(() => {
    const storedFanIn = deriveFanIn(flow.id, "agent", activeEnv);
    const storedFanOut = deriveFanOut(flow.id, "agent", activeEnv);
    const storedRags = deriveRags(flow.id, activeEnv);
    return {
      fanIn: storedFanIn.length > 0 ? storedFanIn : flow.fanIn,
      fanOut: storedFanOut.length > 0 ? storedFanOut : flow.fanOut,
      rags: storedRags.length > 0 ? storedRags : flow.rags,
    };
  }, [flow.id, activeEnv, flow.fanIn, flow.fanOut, flow.rags]);
      ...
      <div className="space-y-6 p-6">
          ...
          <div className="flex flex-wrap items-start justify-between gap-3">
            ...
            {!flow.tags.includes("highcode") && (
              <Button 
                ...
                onClick={() => navigate({ to: "/agents/$id/edit", params: { id: flow.id } })}
              >
        ...
        <Tabs defaultValue="pipeline" className="w-full">
          ...
          <TabsContent value="deploy" className="mt-4">
            <DeploySection flowName={flow.slug} flowId={flow.id} environment={activeEnv} mode="agent" />
          </TabsContent>
          <TabsContent value="observability" className="mt-4">
            <ObservabilitySection flowType="agent" />
          </TabsContent>
        </Tabs>
      </div>
    </AppLayout>
