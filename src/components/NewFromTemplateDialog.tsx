...
import { templatesByKind, type TemplateKind } from "@/data/templates";
...
export function NewFromTemplateDialog({ open, onOpenChange, kind }: Props) {
  ...
  const target = kind === "agent" ? "/agents/new" : "/orchestrations/new";
  ...
  const proceed = () => {
    ...
    if (selected === "blank") {
      ...
      navigate({ to: target, search: { template: selected } });
    }
